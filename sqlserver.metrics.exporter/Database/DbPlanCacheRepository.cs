...
namespace SqlServer.Metrics.Exporter.Database
{
    public class DbPlanCacheRepository : IDbPlanCacheRepository
    {
        ...
        private readonly string getHistoricalPlanCacheQuery = "select dateadd(minute, datediff(minute,  getutcdate(), getdate()),timestamp_utc) as 'timestamp_utc', CAST(event_data AS VARCHAR(MAX)) as event_data FROM sys.fn_xe_file_target_read_file(@xEventPath, NULL, NULL, NULL) where CONVERT(datetime2, timestamp_utc) > dateadd(minute, datediff(minute, getdate(), getutcdate()), @fromUtc);";
        private readonly string getStoredProcedures = "select object_id, obj.Name as 'Name', she.name as 'SchemaName' from sys.objects obj inner join sys.schemas she on she.schema_id = obj.schema_id where obj.type='P'";
        ...
        public async Task<Dictionary<int, string>> GetObjectIdAndProcedureNames()
        {
            ...
            var procedures = await connection.QueryAsync(
                getStoredProcedures,
                commandType: CommandType.Text);
            return procedures.ToDictionary(item => (int)item.object_id, item => $"{(string)item.SchemaName}.{(string)item.Name}");
        }
    }
}